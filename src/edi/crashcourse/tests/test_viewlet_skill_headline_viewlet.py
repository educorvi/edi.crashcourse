# -*- coding: utf-8 -*-
from edi.crashcourse.interfaces import IEdiCrashcourseLayer
from edi.crashcourse.testing import EDI_CRASHCOURSE_FUNCTIONAL_TESTING
from edi.crashcourse.testing import EDI_CRASHCOURSE_INTEGRATION_TESTING
...
class ViewletIntegrationTest(unittest.TestCase):
    ...
    def test_skill_headline_viewlet_is_registered(self):
        view = BrowserView(self.portal['other-document'], self.request)
        manager_name = 'plone.abovecontenttitle'
        alsoProvides(self.request, IEdiCrashcourseLayer)
        ...
        manager.update()
        my_viewlet = [v for v in manager.viewlets if v.__name__ == 'skill-headline-viewlet']  # NOQA: E501
        self.assertEqual(len(my_viewlet), 1)

    # XXX would be nice to have this test working:
    # def test_skill_headline_viewlet_is_not_available_on_newsitem(self):
...
class ViewletFunctionalTest(unittest.TestCase):
    ...
    layer = EDI_CRASHCOURSE_FUNCTIONAL_TESTING
